<template>
  <div class="inventario-view">
    <div class="page-hero">
      <VolverBtn :to="links.inventario.modulo" />
    ...
    <div class="table-card" v-if="vista === 'productos'">
      ...
      <DataTable :value="productosVisibles" data-key="id_producto" :paginator="true" :rows="10" responsiveLayout="scroll" class="mt-4">
        ...
        <Column field="costo" header="Costo unitario" sortable>
          <template #body="slotProps">{{ slotProps.data.costo == null ? 'Sin registrar' : `S/. ${fmt2(slotProps.data.costo)}` }}</template>
        </Column>
        ...
        <Column header="Acciones" :exportable="false" style="min-width: 22rem">
          <template #body="slotProps">
            <div class="row-actions">
              ...
              <Button v-if="esAdmin" :disabled="$saving || ocupado" :icon="slotProps.data.estado ? 'pi pi-ban' : 'pi pi-check'" :severity="slotProps.data.estado ? 'danger' : 'success'" text rounded :title="slotProps.data.estado ? 'Desactivar' : 'Activar'" @click="toggleEstado(slotProps.data)" />
            </div>
          </template>
        </Column>
      </DataTable>
      ...
      <DataTable :value="compras" :paginator="true" :rows="10" class="mt-4">
        ...
        <Column header="Acciones">
          <template #body="slotProps">
            ...
            <Button :disabled="$saving || ocupado" icon="pi pi-trash" severity="danger" text rounded @click="eliminarCompra(slotProps.data)" />
          </template>
        </Column>
      </DataTable>
    </div>
    ...
    <div class="table-card" v-if="vista === 'inversiones'">
      ...
      <DataTable :value="inversiones" :paginator="true" :rows="10" class="mt-4">
        ...
        <Column field="monto" header="Monto" sortable>
          <template #body="slotProps">
            <span :class="{ 'monto-anulado': slotProps.data.estado === 'Anulada' }">S/. {{ fmt2(slotProps.data.monto) }}</span>
          </template>
        </Column>
        <Column field="estado" header="Estado" sortable>
          <template #body="slotProps">
            <Tag :value="slotProps.data.estado || 'Registrada'" :severity="slotProps.data.estado === 'Anulada' ? 'danger' : 'success'" />
          </template>
        </Column>
        <Column header="Acciones">
          <template #body="slotProps">
            <Button :disabled="$saving" icon="pi pi-eye" severity="info" text rounded @click="verInversion(slotProps.data)" />
            <Button v-if="slotProps.data.estado !== 'Anulada'" :disabled="$saving || ocupado" icon="pi pi-ban" severity="danger" text rounded title="Anular inversión" aria-label="Anular inversión" @click="eliminarInversion(slotProps.data)" />
          </template>
        </Column>
      </DataTable>
    </div>
    ...
    <Dialog v-model:visible="productoDialog" :header="editing.id_producto ? 'Editar Producto' : 'Nuevo Producto'" :modal="true" :style="{ width: '520px' }">
      <div class="formgrid grid">
        ...
        <div class="field col-6">
          ...
          <InputNumber :maxFractionDigits="3" placeholder="Ej. 80.00" id="costo" v-model="form.costo" mode="currency" currency="PEN" locale="es-PE" :min="0" class="w-full" />
        </div>
        ...
        <div class="field col-6" v-if="editing.id_producto && form.unidad_medida">
          <label for="stock-actual">Stock actual ({{ unidadLabel(form.unidad_medida) }})</label>
          <div class="stock-actual">{{ fmtStock(editing.stock) }} {{ unidadLabel(editing.unidad_medida) }}</div>
        </div>
        <div class="field col-12">
      ...
      <template #footer>
        ...
        <Button :disabled="$saving || procesandoOperacion" :loading="procesandoOperacion" label="Guardar" @click="guardarProducto" />
      </template>
    </Dialog>
    ...
    <Dialog v-model:visible="stockDialog" :header="stockModo === 'entrada' ? 'Agregar stock' : 'Registrar salida'" :modal="true" :style="{ width: '460px' }">
      <div class="formgrid grid">
        ...
        <div class="field col-12" v-if="stockModo === 'entrada'">
          ...
          <InputText id="stock-motivo" v-model="stockForm.motivo" maxlength="255" placeholder="Ej. Reposición de almacén" class="w-full" />
        </div>
        <div class="field col-12" v-else>
          <label for="stock-motivo">Motivo *</label>
          <InputText id="stock-motivo" v-model="stockForm.motivo" maxlength="255" placeholder="Ej. Preparación de menú del día" class="w-full" />
        </div>
      </div>
    ...
    <Dialog v-model:visible="productoVerDialog" header="Detalle de producto" :modal="true" :style="{ width: '480px' }">
      <div v-if="productoVisto.id_producto" class="detalle-info">
        ...
        <div class="d-row"><span class="d-label">Costo unitario</span><span>{{ productoVisto.costo == null ? 'Sin registrar' : `S/. ${fmt2(productoVisto.costo)}` }}</span></div>
    ...
    <Dialog v-model:visible="inversionDialog" header="Registrar inversión" :modal="true" :style="{ width: '520px' }">
      ...
      <template #footer>
        ...
        <Button :disabled="$saving || procesandoOperacion" :loading="procesandoOperacion" label="Guardar" @click="guardarInversion" />
      </template>
    </Dialog>
    ...
    <Dialog v-model:visible="inversionDetalleDialog" header="Detalle de inversión" :modal="true" :style="{ width: '480px' }">
      <div v-if="compraSeleccionada.id_inversion" class="detalle-info">
        ...
        <div class="d-row"><span class="d-label">Fecha</span><span>{{ fmtFecha(compraSeleccionada.fecha) }}</span></div>
        <div class="d-row"><span class="d-label">Notas</span><span>{{ compraSeleccionada.notas || '-' }}</span></div>
        <div class="d-row"><span class="d-label">Estado</span><span>{{ compraSeleccionada.estado || 'Registrada' }}</span></div>
        <div v-if="compraSeleccionada.fecha_anulacion" class="d-row"><span class="d-label">Anulada el</span><span>{{ fmtFecha(compraSeleccionada.fecha_anulacion) }}</span></div>
      </div>
    </Dialog>
  </div>
</template>
...
const procesandoOperacion = ref(false)
const ocupado = computed(() => procesandoStock.value || procesandoCompra.value || procesandoOperacion.value)
...
const agregarDialog = () => {
  ...
  form.value = { nombre: '', unidad_medida: null, estado: true, id_categoria: null, precio: null, costo: null, stock: null, descripcion: '' }
...
const guardarProducto = async () => {
  ...
  try {
    ...
    procesandoOperacion.value = false
  }
}
...
const eliminarInversion = async (inv) => {
  if (procesandoOperacion.value) return
  if (!confirm(`¿Anular la inversión "${inv.descripcion}"? Dejará de contar en los totales, pero se conservará en el historial.`)) return
  procesandoOperacion.value = true
  try {
    await api.delete(`/inventario/inversiones/${inv.id_inversion}`)
    toast.add({ severity: 'success', summary: 'Inversión anulada', life: 2500 })
    await Promise.all([cargarInversiones(), cargarResumen()])
  } catch (e) {
    toast.add({ severity: 'error', summary: e.response?.data?.error || 'No se pudo anular', life: 3500 })
  } finally {
    procesandoOperacion.value = false
  }
}
...
<style scoped>
.monto-anulado { text-decoration: line-through; color: var(--text-muted); }
