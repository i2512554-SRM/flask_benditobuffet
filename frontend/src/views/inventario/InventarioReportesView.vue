  ...
  <div>
    ...
    <div class="stats-grid">
      <div class="stat-card">
        ...
        <div class="stat-content">
          ...
          <span v-if="resumen.productos_sin_costo" class="stat-note">{{ resumen.productos_sin_costo }} sin costo registrado</span>
        </div>
      </div>
      <div class="stat-card">
    ...
    <div class="charts-grid" v-if="!loading && !error">
      <section class="chart-card">
        ...
        <DonaChart :items="estadosItems" :moneda="false" :mostrarTotal="false" :altura="190" />
      </section>
      <section class="chart-card">
        <h3>Valor del stock por categoría</h3>
        <BarChart :categorias="categoriasValor" :series="seriesValor" horizontal :altura="valorAltura" />
        <p v-if="valorCategorias.sin_costo" class="chart-nota">{{ valorCategorias.sin_costo }} producto(s) sin costo unitario registrado no se incluyen en el valor por categoría.</p>
      </section>
    </div>
...
const valorCategorias = ref({ categorias: [], sin_costo: 0, con_costo: 0 })

const formatMoney = (val) => Number(val || 0).toLocaleString('es-PE', { minimumFractionDigits: 2 })

const estadosItems = computed(() => {
  const s = resumen.value.stock_estados || { disponibles: 0, stock_bajo: 0, agotados: 0 }
  return [
    { etiqueta: 'Disponible', valor: s.disponibles, color: '#16a34a' },
    { etiqueta: 'Stock bajo', valor: s.stock_bajo, color: '#f59e0b' },
    { etiqueta: 'Agotado', valor: s.agotados, color: '#dc2626' }
  ]
})

const categoriasValor = computed(() => (valorCategorias.value.categorias || []).map((c) => c.categoria))
const seriesValor = computed(() => [{
  label: 'Valor del stock',
  valores: (valorCategorias.value.categorias || []).map((c) => c.valor_stock),
  color: '#8b5cf6'
}])
const valorAltura = computed(() => Math.max(220, 48 * (categoriasValor.value.length || 1)))

const load = async () => {
  loading.value = true
  try {
    error.value = ''
    const [res, prod, mov, val] = await Promise.all([
      api.get('/inventario/resumen'),
      api.get('/inventario/productos'),
      api.get('/inventario/movimientos'),
      api.get('/inventario/valor-stock')
    ])
    productos.value = prod.data.data || []
    movimientos.value = mov.data.data || []
    if (res.data.success) resumen.value = res.data.data
    if (val.data.success) valorCategorias.value = val.data.data
  } catch (err) {
...
.stat-note { font-size: 0.68rem; color: var(--text-muted); margin-top: 0.2rem; }
