...
<p>{{ rango }}</p>
      ...
      <div v-if="!loading && salarios.length" class="table-card">
        <DataTable :value="salarios" :paginator="true" :rows="10" stripedRows dataKey="id_usuario" class="mt-4 animated-table">
          ...
          <Column header="Saldo pendiente"><template #body="{ data }">
            ...
            <router-link v-if="data.pagos_por_clasificar" :to="links.personal.pagos">Revisar pagos</router-link>
    ...
    <Dialog v-model:visible="anulacionVisible" header="Anular descuento" modal :style="{ width: 'min(95vw, 420px)' }">
      ...
      <Button label="Confirmar anulación" :disabled="$saving || anulando" :loading="anulando" @click="anular" />
    </Dialog>
    <!-- Estado vacío -->
    <Transition name="fade">
...
async function guardar() {
  ...
  try {
    ...
    toast.add({ severity: 'error', summary: 'No se pudo guardar', detail: e.response?.data?.error || 'Intenta nuevamente', life: 4000 })
...
async function anular() {
  ...
  try {
    ...
    toast.add({ severity: 'error', summary: 'No se pudo anular', detail: e.response?.data?.error || 'Intenta nuevamente', life: 3000 })
  } finally {
