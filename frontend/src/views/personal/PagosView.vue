<template>
  <div class="pagos-view">
    ...
    <div v-if="costoMensual.length" class="chart-card">
      <h2>Costo de personal · últimos 12 meses</h2>
      <BarChart :categorias="costoCategorias" :series="costoSeries" :altura="260" />
    </div>
...
const hoyLima = fechaLocal()
const [anioLima, mesLima] = hoyLima.split('-').map(Number)
...
const empleados = ref([])
const costoMensual = ref([])
const dialogVisible = ref(false)
const modal = ref('pago')
const vista = ref('empleados')
const mesSeleccionado = ref(new Date(`${hoyLima}T12:00:00`))
const form = ref({})
...
function abrirActualizacion(pago) {
  ...
  actualizacion.value = { tipo: pago.tipo || null, estado: pago.estado === 'Pendiente' ? 'Pagado' : pago.estado,
    semana: pago.semana || fechaLocal(pago.fecha) }
...
async function guardarActualizacion() {
  ...
  try {
    ...
    toast.add({ severity: 'error', summary: 'No se pudo actualizar', detail: e.response?.data?.error || 'Intenta nuevamente.', life: 4000 })
...
const etiquetaMes = (m) => {
  ...
  const alto = d.toLocaleString('es-PE', { month: 'short' })
  return alto.charAt(0).toUpperCase() + alto.slice(1)
}

const costoCategorias = computed(() => costoMensual.value.map(etiquetaMes))
const costoSeries = computed(() => [
  { label: 'Pagado', valores: costoMensual.value.map((m) => m.pagado), color: '#16a34a' },
  { label: 'Adelantos', valores: costoMensual.value.map((m) => m.adelantos), color: '#f59e0b' }
])

const cargarCostoMensual = async () => {
  try {
    const res = await api.get('/personal/costo-mensual')
    if (res.data.success) costoMensual.value = res.data.data || []
  } catch (err) {
    console.error('Error loading costo mensual:', err)
  }
}
...
const cargarDatos = async () => {
  loading.value = true
  try {
    const mes = mesSeleccionado.value ? mesSeleccionado.value.getMonth() + 1 : mesLima
    const anio = mesSeleccionado.value ? mesSeleccionado.value.getFullYear() : anioLima
...
const guardar = async () => {
  ...
  if (!form.value.fecha) {
    toast.add({ severity: 'warn', summary: 'Seleccione una fecha', life: 3000 })
    return
  }
  ...
  try {
    ...
    toast.add({ severity: 'error', summary: err.response?.data?.error || 'No se pudo registrar', life: 4000 })
  } finally {
...
onMounted(async () => {
  ...
  await Promise.all([cargarDatos(), cargarEmpleados(), cargarCostoMensual()])
})
...
.chart-card { background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 14px; padding: 1.25rem; box-shadow: var(--shadow-soft); margin-bottom: 1.25rem; }
.chart-card h2 { margin: 0 0 0.75rem; font-size: 1.05rem; color: var(--text-main); }
