    ...
    <Transition name="fade-up">
      <div v-if="!loading" class="table-card">
        <DataTable :value="empleados" :paginator="true" :rows="10" :filters="filtros"
          ...
          <template #empty><EstadoVacio v-if="loading" compacto titulo="Revolviendo los datos…" expresion="pensando" /><EstadoVacio v-else compacto titulo="Aún no hay empleados registrados" mensaje="Agrega al primero con el botón de arriba." expresion="feliz" /></template>
        </DataTable>
      </div>
    </Transition>
...
const guardar = async () => {
  ...
  try {
    ...
    toast.add({ severity: 'error', summary: err.response?.data?.error || 'Error al guardar', life: 4000 })
  } finally {
...
const cambiarEstado = async (emp, estado) => {
  ...
  try {
    ...
    toast.add({ severity: 'error', summary: 'No se pudo cambiar el estado', detail: err.response?.data?.error, life: 4000 })
  }
}
</script>
